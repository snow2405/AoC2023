...
int main()
{
    ...
    vector<vector<bool>> IsPath(fileContent.size(), vector<bool>(fileContent[0].size(), false));

    while (fileContent[xPos][yPos] != 'S')
    {
        IsPath[xPos][yPos] = true;
    ...
    for (size_t i = 0; i < IsPath.size(); i++)
    {
        for (size_t j = 0; j < IsPath[i].size(); j++)
        {
            if (!IsPath[i][j])
            {
                int counter = 0;
                for (int x = 0; x < j; x++)
                {
                    if (IsPath[i][x])
                    {
                        if (fileContent[i][x] == '|' ||fileContent[i][x] == 'L' ||fileContent[i][x] == 'J')
                        {
                            counter++;
                        }
                    }
    ...
    cout << "Total filled: " << filledTotal << endl;
    return 0;
}