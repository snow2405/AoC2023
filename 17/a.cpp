...
vector<Cell> getValidNeighbours(const Position &pos, const vector<vector<int>> &grid, int maxStraight)
{
    ...
    for (Direction dir : getDirections(pos.dir))
    {
        if (pos.dir != NONE && pos.dir == dir && pos.straight >= maxStraight)
            continue;
        else if (pos.dir != NONE && pos.dir != dir && pos.straight < 1)
            continue; // Allow changing direction only after 1 straight move
...
int dijkstraSearch(const vector<vector<int>> &grid, const Position &start, const Position &end, int maxStraight)
{
    ...
    while (!queue.empty())
    {
        ...
        for (Cell &neighbour : getValidNeighbours(cur.position, grid, maxStraight))
        {
...
int main()
{
    ...
    int shortestPathCost = dijkstraSearch(grid, start, end, 3);
